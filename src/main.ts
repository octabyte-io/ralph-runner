...
import type { CliOverrides, RalphConfig } from './config.ts'
...
Usage: ralph [flags]
       ralph init            scaffold .ralph/config.json + .ralph/prompt.md
  ...
  -n, --concurrency <int>   parallel agents (default 3)
      ...
      --smoke               also run the smoke command at merge verification
      ...
      --verify-cmd <cmd>    override the merge verification command
  -h, --help

Config: .ralph/config.json (see 'ralph init'); flags win over config.
...
async function main(): Promise<void> {
  ...
  const cli: CliOverrides = {
    concurrency: values.concurrency !== undefined ? Number(values.concurrency) || undefined : undefined,
    chrome: values.chrome ? 'on' : values['no-chrome'] ? 'off' : undefined,
    ...
    issues: values.issues
      ...
      : undefined,
    maxMinutes: values['max-minutes'] !== undefined ? Number(values['max-minutes']) || undefined : undefined,
    smoke: values.smoke,
  ...
  const mainSha = await runOk('git', ['rev-parse', '--short', cfg.mainBranch], { cwd: repoRoot })
  const effectiveConcurrency = Math.min(cfg.concurrency, cfg.issues?.length ?? Number.POSITIVE_INFINITY)
  const ui = createUI(cfg, `${cfg.mainBranch}@${mainSha}`, effectiveConcurrency)
...
async function preflight(cfg: RalphConfig): Promise<void> {
  ...
  if (branch !== cfg.mainBranch) fail(`must run from ${cfg.mainBranch} (currently on '${branch}')`)

  const dirty = await runOk('git', ['status', '--porcelain', '-uno'], { cwd: cfg.repoRoot })
  if (dirty) fail(`${cfg.mainBranch} checkout has uncommitted changes — commit or stash first:\n${dirty}`)
  ...
  const stale = await scanStaleWorktrees(cfg)
  if (stale.length > 0) {
    if (!cfg.clean) {
      fail(
        `stale worktrees from a previous run:\n` +
          stale.map((s) => `  ${s.path} (${s.commits} commit(s) ahead of ${cfg.mainBranch})`).join('\n') +
    ...
    for (const s of stale) {
      await removeWorktree(cfg, s.issue)
      await deleteBranch(cfg, s.issue, true)
...
async function dryRun(cfg: RalphConfig): Promise<void> {
  const tracker = new IssueTracker(cfg.ghCmd, cfg.repoRoot, cfg.labels.ready)
  const all = await tracker.listReady()
  const scoped = cfg.issues ? all.filter((i) => cfg.issues!.includes(i.number)) : all
  const workable = scoped.filter((i) => isWorkable(i, cfg.labels))
  const skipped = scoped.filter((i) => !isWorkable(i, cfg.labels))
  ...
  for (const issue of sortByPriority(workable, cfg.priority)) {
    ...
    lines.push(`  #${issue.number}  [p${priorityRank(issue, cfg.priority)}] ${status.padEnd(24)} ${issue.title}`)
  }
  ...
  lines.push(`  git worktree add ${join(cfg.worktreesDir, 'issue-N')} -b ralph/issue-N ${cfg.mainBranch} && ${cfg.setupCommand}`)
  ...
  lines.push(`  git rebase ${cfg.mainBranch} && git merge --ff-only && ${cfg.verifyCommand}${cfg.smoke ? ` && ${cfg.smokeCommand}` : ''} && gh issue close`)
